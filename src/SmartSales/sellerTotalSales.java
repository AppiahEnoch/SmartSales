...
String ID=ShareData.userID_;
    ...
    private void getToday() {
        DBcon();
        openConn(conn);
        qry = "select sum(sales.discountOnTotalCost)" +
                " as amt " +
                "from sales where date(time)=curdate() " +
                "and sales.userID='"+ID+"'";
        try {
    ...
    private void getWeekSales() {
        ...
        DBcon();
        openConn(conn);
        qry ="select sum(sales.discountOnTotalCost) as amt from sales where " +
                "yearweek(time,1) = yearweek(curdate(),1)   and sales.userID= '"+ID+"'";
...
//"+ID+"'
    ...
    private void getMonthSales() {
        ...
        DBcon();
        openConn(conn);
        qry ="select sum(sales.discountOnTotalCost) as amt from sales " +
                "where month(time) = month(curdate())   and sales.userID= '"+ID+"'";
    ...
    private void getYearSales() {
        ...
        openConn(conn);
        qry ="select sum(sales.discountOnTotalCost) as amt from sales " +
                "where year(time) = year(curdate())   and sales.userID= '"+ID+"'";
    ...
    private void getLastYearSales() {
        ...
        DBcon();
        openConn(conn);
        qry ="select sum(sales.discountOnTotalCost) as amt from " +
                "sales where year(time)= year(date_sub(curdate(),interval 1 Year))" +
                " and sales.userID='"+ID+"'";
        try {
            st = conn.createStatement();
            rs = st.executeQuery(qry);
    ...
    private void getLastMonth() {
        ...
        DBcon();
        openConn(conn);
        qry ="select sum(sales.discountOnTotalCost) as amt from sales where year(time)=year(current_date -interval 1 month) " +
                " and month(time)=month(current_date - interval 1 month) and sales.userID='"+ID+"' ";
        try {
            st = conn.createStatement();
            rs = st.executeQuery(qry);
    ...
    private void getLastWeek() {
        ...
        DBcon();
        openConn(conn);
        qry ="select sum(sales.discountOnTotalCost) as amt from sales where yearweek(time)=yearweek(now()- interval 1 week)" +
                " and sales.userID='"+ID+"' ";
        try {
            st = conn.createStatement();
            rs = st.executeQuery(qry);
    ...
    private void getMyOwnData() {
        ...
        DBcon();
        openConn(conn);
        qry ="select sum(sales.discountOnTotalCost) as amt from sales where time >=date(now()) - interval "+tfValue+" day " +
                " and sales.userID='"+ID+"' ";
        try {
            st = conn.createStatement();
            rs = st.executeQuery(qry);
